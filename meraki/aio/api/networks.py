class AsyncNetworks:
    ...
    def getNetworkEnvironmentalEvents(self, networkId: str, total_pages=1, direction='next', **kwargs):
        ...
        **List the environmental events for the network**
        ...
        query_params = ['includedEventTypes', 'excludedEventTypes', 'sensorSerial', 'gatewaySerial', 'perPage', 'startingAfter', 'endingBefore', 't0', 't1', 'timespan', ]
        params = {k.strip(): v for k, v in kwargs.items() if k.strip() in query_params}
    ...
    def getNetworkEnvironmentalEventsEventTypes(self, networkId: str):
        ...
        **List the event type to human-readable description**
        ...
        metadata = {
            'tags': ['networks', 'monitor', 'environmental', 'events', 'eventTypes'],
    ...
    def updateNetworkLocationScanning(self, networkId: str, **kwargs):
        ...
        **Change scanning API settings**
        ...
        body_params = ['analyticsEnabled', 'scanningApiEnabled', ]
        payload = {k.strip(): v for k, v in kwargs.items() if k.strip() in body_params}
    ...
    def updateNetworkLocationScanningHttpServers(self, networkId: str, endpoints: list):
        """
    ...
    def getNetworkSensors(self, networkId: str, total_pages=1, direction='next', **kwargs):
        """
        **List all sensors in a given network.**
        https://developer.cisco.com/meraki/api-v1/#!get-network-sensors
        ...
        - startingAfter (string): A token used by the server to indicate the start of the page. Often this is a timestamp or an ID but it is not limited to those. This parameter should not be defined by client applications. The link for the first, last, prev, or next page in the HTTP Link header should define it.
        - endingBefore (string): A token used by the server to indicate the end of the page. Often this is a timestamp or an ID but it is not limited to those. This parameter should not be defined by client applications. The link for the first, last, prev, or next page in the HTTP Link header should define it.
        - alertingOn (string): The type of reading that the sensor should be alerting on. Can be one of 'temperature', 'humidity', 'water_detection' or 'door'
        - alertProfiles (array): The list of environmental alert profile names that the sensor is bound to.
        """

        kwargs.update(locals())

        metadata = {
            'tags': ['networks', 'configure', 'sensors'],
            'operation': 'getNetworkSensors'
        }
        resource = f'/networks/{networkId}/sensors'

        query_params = ['perPage', 'startingAfter', 'endingBefore', 'alertingOn', 'alertProfiles', ]
        params = {k.strip(): v for k, v in kwargs.items() if k.strip() in query_params}

        array_params = ['alertProfiles', ]
        for k, v in kwargs.items():
            if k.strip() in array_params:
                params[f'{k.strip()}[]'] = kwargs[f'{k}']
                params.pop(k.strip())
    ...
    def updateNetworkSettings(self, networkId: str, **kwargs):
        ...
        **Update the settings for a network**
        ...
        body_params = ['localStatusPageEnabled', 'remoteStatusPageEnabled', 'secureConnect', 'fips', 'localStatusPage', ]
        payload = {k.strip(): v for k, v in kwargs.items() if k.strip() in body_params}
